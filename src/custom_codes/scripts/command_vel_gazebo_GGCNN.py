...
def main():
    ...
    posBase, oriBase = ur5_vel.tf.lookupTransform("base_link", "object_detected", rospy.Time())
    posCam, oriCam = ur5_vel.tf.lookupTransform("camera_depth_optical_frame", "object_detected", rospy.Time())
    # msg = rospy.wait_for_message('/ggcnn/out/command', Float32MultiArray)
    # d = list(msg.data)
    ori = euler_from_quaternion(oriCam)
    print("Ang: ", ori[-1])

    ur5_vel.add_sphere(posBase, 0.05, ColorRGBA(0.0, 1.0, 0.0, 1.0))

    joint_values = get_ik([posBase[0], posBase[1], posBase[2] + 0.2])
