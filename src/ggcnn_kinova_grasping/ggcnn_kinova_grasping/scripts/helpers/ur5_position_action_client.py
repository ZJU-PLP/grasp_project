...
def move_to_position(position = None, orientation = None):
    """Send a cartesian goal to the action server."""
    global position_client
    if position_client is None:
        init()

    # joint_values = [3.14, -1.5707, 0, -1.5707, -1.5707, 0]
    joint_values = get_ik(position, orientation)
...
def init():
    global position_client
    position_client = actionlib.SimpleActionClient('arm_controller/follow_joint_trajectory', FollowJointTrajectoryAction)
